...
# VARSITY = ['Aarush', 'Nathaniel', 'Cole'] testing code
VARSITY = ['Sydney', 'Jasmine', 'Anna', 'Sophie', 'Lucy', 'Olivia', 'Olivia', 'Madalynn', 'Leah', 'Erika', 'Audry', 'Cole', 'Aarush', 'Nathaniel', 'Jaden', 'Kaden', 'Sebasti�n', 'Jessetyler', 'Cohen', 'Joe']
tournament_results = []
...
profile_urls = [data[-1] for data in profile_data if data[-2].lower() != 'pre-freshman' and data[0] in VARSITY]

def scrape_matches(event_block, profile_url, tournament_name):
    """Extracts match results only from the Tim King tournament."""
    ...
    for match in match_cards:
        try:
            ...
            team1_players = match.locator(".d-none.d-md-block .team").nth(0).locator(".player-details a.flex-column.player-name").all()
            team2_players = match.locator(".d-none.d-md-block .team").nth(1).locator(".player-details a.flex-column.player-name").all()

            # Join the names for each team
            team_1_names = '/'.join([player.inner_text().strip() for player in team1_players])
            team_2_names = '/'.join([player.inner_text().strip() for player in team2_players])
...
with sync_playwright() as p:
    ...
    for profile_url in profile_urls:
        ...
        if 'singles' not in current_selection:
            dropdown_anchor = page.locator("div.popovermenu-anchor.resultsTab__anchorClass__PGL2O.pl8").first
        ...
        for event in event_blocks:
            try:
                tournament_name = event.locator(".eventItem__eventName__6hntZ span:not(.eventItem__middleDot__1ttwt)").inner_text().strip()
